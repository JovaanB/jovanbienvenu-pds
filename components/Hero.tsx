  ...
  return (
    <header className="relative min-h-screen flex flex-col items-center justify-center px-4 pt-24 pb-16 z-10">
      <div className="max-w-4xl mx-auto text-center flex flex-col items-center gap-8">
        {/* Main headline */}
        <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight text-white leading-[1.05]">
        ...
        {/* Mini-stats */}
        <div className="w-full flex items-stretch mt-4 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm overflow-hidden divide-x divide-white/10">
          {hero.stats.map(({ value, label }) => {
            ...
            const suffix = value.includes("%")
              ...
              : value.includes("+")
                ...
                : "";
            return (
              <div key={label} className="flex flex-col items-center justify-center flex-1 px-4 sm:px-10 py-4 sm:py-5 gap-1">
                <span className="text-3xl sm:text-4xl font-bold leading-none text-white">
                  {num}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-300">
                    {suffix}
                  </span>
                </span>
                <span className="text-[10px] sm:text-[11px] font-semibold uppercase tracking-widest text-slate-500 text-center">
                  {label}
                </span>
              </div>
            );
