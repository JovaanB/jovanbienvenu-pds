          ...
          {cards.map(
            (
              ...
              <FadeIn key={data.title} delay={i * 120}>
                <div
                  className={`group relative bg-surface-dark border border-white/[0.06] hover:${accentBorder} rounded-2xl p-8 md:p-10 overflow-hidden card-hover h-full transition-all duration-300`}
                >
                  ...
                  <div className="absolute top-4 right-4 opacity-[0.04] group-hover:opacity-[0.09] transition-opacity duration-500 pointer-events-none">
                    <data.Icon
                      size={130}
                      className={`${accentIcon} rotate-6`}
                    />
                  </div>

                  {/* Top glow on hover */}
                  <div
                    className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r ${glowColor} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                  />
                  ...
                  <div className="relative z-10 flex flex-col gap-7">
                    ...
                    <div className="flex items-center justify-between">
                      <div
                        className={`w-12 h-12 rounded-xl ${accentBg} flex items-center justify-center ${accentIcon} border ${accentBorder}`}
                      >
                        <data.Icon size={24} />
                      </div>
                      <span
                        className={`text-[11px] font-bold px-3 py-1.5 rounded-full border ${badgeClass} uppercase tracking-wide`}
                      >
                        {badge}
                      </span>
                    </div>
                    ...
                    <ul className="space-y-2.5">
                      {data.features.map((f) => (
                        ...
                        >
                          <CheckCircle
                            size={15}
                            className={`${checkColor} flex-shrink-0 mt-0.5`}
                          />
                          {f}
                        </li>
                      ))}
